...
## Corpus Check
- 50 files · ~111,499 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 690 nodes · 915 edges · 84 communities (35 shown, 49 thin omitted)
...
- test_find_battery_path_skips_no_threshold_file
- test_find_battery_path_multi_battery_first_wins
- test_find_battery_path_found

## God Nodes (most connected - your core abstractions)
1. `ThresholdWindow` - 55 edges
2. `Config` - 35 edges
3. `_mock_power_supply_root()` - 17 edges
4. `read_sysfs()` - 16 edges
5. `TrayIcon` - 16 edges
6. `FakeSettings` - 16 edges
7. `BatteryGuardApp` - 13 edges
8. `FakeSettings` - 13 edges
9. `_root()` - 13 edges
10. `Decision` - 11 edges
...
Cohesion: 0.11
Nodes (19): _mock_power_supply_root(), Returns the battery even without a threshold file., Returns None when power_supply directory is empty., Properly filters through Mains, UPS, and finds Battery., Returns the first Battery-type entry by sort order., Returns the battery even when it lacks the threshold file., Returns None when power_supply directory is empty., Monkeypatch Path.iterdir on /sys/class/power_supply to return items from… (+11 more)
...
### Community 53 - "Decision"
Cohesion: 0.10
Nodes (19): 10. Component vehicles: shipped vs custom vs tokens, 1. Layout: Industrial grid, scroll-free, 2. Window chrome: full web, no GTK HeaderBar, 3. Bridge: WebKitGTK script message handlers + UserScript shim, 4. Build chain: commit Vite dist to `web/dist`, 5. Theme: White ↔ Gray 100, parity with system dark mode, 6. Tray + notifications: stay native, 7. i18n: English-only v1 with `t()` stub (+11 more)

### Community 54 - "Carbon component inventory — Industrial Grid layout"
Cohesion: 0.18
Nodes (10): Carbon component inventory — Industrial Grid layout, Charge Limit panel, Cross-cutting, Flagged sub-decisions (input to the architecture ADR), Footer, Header, Lower settings grid (three panels), Mock divergences — do not copy the prototype here (+2 more)

### Community 69 - "Q: query it says that charging is fair on the AC adapter why?"
Cohesion: 0.40
Nodes (4): Answer, Outcome, Q: query it says that charging is fair on the AC adapter why?, Source Nodes

## Knowledge Gaps
- **93 isolated node(s):** `universalDKMSModuleUninstaller.sh script`, `prettier`, `Reference documents`, `1. Layout: Industrial grid, scroll-free`, `2. Window chrome: full web, no GTK HeaderBar` (+88 more)
...
- **What connects `universalDKMSModuleUninstaller.sh script`, `prettier`, `Reference documents` to the rest of the system?**
